...
COHORT = "nlsy79"
COHORT_CONFIG = "config/nlsy79.yml"

# The extracted NLSY79 occupation values are stored as 3-digit integers such as
# 245 and 984. These align with Census 2002 occupation codes with the trailing
# zero dropped, so 245 maps to 2450 and 984 maps to 9840.
...
def run_nlsy79_occupation_major_groups(
    *,
    root: Path,
    summary_output_path: Path = Path("outputs/tables/nlsy79_occupation_major_group_summary.csv"),
    model_output_path: Path = Path("outputs/tables/nlsy79_high_skill_occupation_outcome.csv"),
    min_class_n: int = 50,
    ...
    if not source_path.exists():
        ...
        if missing:
            ...
            df["__occupation_group"] = _assign_major_group(df["occupation_code_2000"])

            rows: list[dict[str, Any]] = []
            used = df.loc[df["__occupation_group"].notna()].copy()
            for group_key, group_label, low, high in MAJOR_GROUPS:
                ...
                row: dict[str, Any] = {
                    ...
                    "mean_household_income": float(pd.to_numeric(grp.get("household_income"), errors="coerce").mean()) if not grp.empty and "household_income" in grp.columns else pd.NA,
                    "mean_net_worth": float(pd.to_numeric(grp.get("net_worth"), errors="coerce").mean()) if not grp.empty and "net_worth" in grp.columns else pd.NA,
                    "source_data": source_data,
                }
                for col in SUMMARY_COLUMNS:
                    row.setdefault(col, pd.NA)
            ...
            work = pd.DataFrame(
                {
                    ...
                    "age": pd.to_numeric(used["age_2000"], errors="coerce"),
                }
            ).dropna()
            if work.empty:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build NLSY79 occupation major-group summaries and a high-skill occupation proxy model.")
    parser.add_argument("--project-root", type=Path, default=project_root(), help="Project root path.")
    parser.add_argument("--summary-output-path", type=Path, default=Path("outputs/tables/nlsy79_occupation_major_group_summary.csv"))
    parser.add_argument("--model-output-path", type=Path, default=Path("outputs/tables/nlsy79_high_skill_occupation_outcome.csv"))
    parser.add_argument("--min-class-n", type=int, default=50)
    args = parser.parse_args(argv)

    root = Path(args.project_root)
    summary, model = run_nlsy79_occupation_major_groups(
        root=root,
